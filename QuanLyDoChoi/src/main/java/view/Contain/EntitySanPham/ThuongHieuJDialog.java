...
public class ThuongHieuJDialog extends javax.swing.JDialog {
    ...
    ThuongHieuServiceImpl thsv = new ThuongHieuServiceImpl();
    SanPhamJPanel spj;
    public ThuongHieuJDialog(JPanel parent, boolean modal) {
        initComponents();
        spj=(SanPhamJPanel) parent;
        setLocationRelativeTo(null);
        fillTable(thsv.getAll());
    }

    void fillTable(List<ThuongHieu> listThuongHieu) {
        model = (DefaultTableModel) Table.getModel();
        model.setRowCount(0);
        int i=0;
        for (ThuongHieu thuongHieu : listThuongHieu) {
            i++;
            model.addRow(new Object[]{i,thuongHieu.getMaThuongHieu(),thuongHieu.getTenThuongHieu()});
        }
    }

    void showData(int index) {
        ThuongHieu th = thsv.getAll().get(index);
    ...
    private String gennerateMa() {
        ThuongHieu sp = thsv.selectTop1DESC();
    ...
    private ThuongHieu readForm() {
        ...
        if (count == 0) {
            return new ThuongHieu(ma, ten, Auth.idNhanVien(), false);
        } else {
            return null;
        }
    }

    private ThuongHieu updateForm() {
        LocalDate toDay = this.getToDay();
        return new ThuongHieu(txtTen.getText(), toDay, Auth.idNhanVien());
    }

    void setForm(ThuongHieu th) {
        txtTen.setText(th.getTenThuongHieu());
    }
    ...
    private void initComponents() {
        ...
        jLabel4.setText("Thêm Thương Hiệu");
    ...
    private void btnThemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnThemActionPerformed
        // TODO add your handling code here:
        ThuongHieu sp = readForm();
        if (sp != null) {
            int confirm = JOptionPane.showConfirmDialog(this, "Bạn Có Muốn Thêm Không?");
            if (confirm == 0) {
                if (thsv.addThuongHieu(sp) > 0) {
                    fillTable(thsv.getAll());
                    spj.fillComboBoxThuongHieu();
                    txtTen.setText("");
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                ThuongHieuJDialog dialog = new ThuongHieuJDialog(new javax.swing.JPanel(), true);
