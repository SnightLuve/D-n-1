...
public class DonViTinhJDialog extends javax.swing.JDialog {
    ...
    int index = -1;
    DonViTinhService dvtsv = new DonViTinhServiceImpl();
    DefaultTableModel model;
    SanPhamJPanel spj;
    public DonViTinhJDialog(JPanel parent, boolean modal) {
        initComponents();
        spj=(SanPhamJPanel) parent;
        fillTable(dvtsv.getAll());
        setLocationRelativeTo(null);
    }

    void fillTable(List<DonViTinh> list) {
        model = (DefaultTableModel) Table.getModel();
        model.setRowCount(0);
        int i=0;
        for (DonViTinh dvt : list) {
            i++;
            model.addRow(new Object[]{i,dvt.getMaDonViTinh(),dvt.getTenDonViTinh()});
        }
    }

    void showData(int index) {
        DonViTinh dvt = dvtsv.getAll().get(index);
        txtTen.setText(dvt.getTenDonViTinh());
    }
    
    //Tự Động GenerateMa
    private String gennerateMa() {
        DonViTinh dvt = dvtsv.selectTop1DESC();
        String maSub = dvt.getMaDonViTinh().substring(3, dvt.getMaDonViTinh().indexOf(" "));
        Integer soVC = Integer.parseInt(maSub);
        String maFinal = "DVT" + (soVC + 1);
        return maFinal;
    }
    private DonViTinh readForm() {
        ...
        if (count == 0) {
            return new DonViTinh(ma,ten, Auth.idNhanVien(), false);
        } else {
            return null;
        }
    }

    DonViTinh updateForm() {
        LocalDate toDay = this.getToDay();
        return new DonViTinh(txtTen.getText(),toDay,Auth.idNhanVien());
    }
    ...
    private void initComponents() {
        ...
        jLabel4.setText("Thêm Đơn Vị Tính");
    ...
    private void btnThemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnThemActionPerformed
        // TODO add your handling code here:
        DonViTinh dvt = readForm();
        if (dvt != null) {
            int confirm = JOptionPane.showConfirmDialog(this, "Bạn Có Muốn Thêm Không?");
            if (confirm == 0) {
                if (dvtsv.addDonViTinh(dvt) > 0) {
                    fillTable(dvtsv.getAll());
                    spj.fillComboBoxDonViTinh();
                    txtTen.setText("");
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                DonViTinhJDialog dialog = new DonViTinhJDialog(new javax.swing.JPanel(), true);
