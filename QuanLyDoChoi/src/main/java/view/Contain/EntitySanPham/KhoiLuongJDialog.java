...
public class KhoiLuongJDialog extends javax.swing.JDialog {
    ...
    KhoiLuongServiceImpl klsv = new KhoiLuongServiceImpl();
    SanPhamJPanel spj;
    public KhoiLuongJDialog(JPanel parent, boolean modal) {
        initComponents();
        spj=(SanPhamJPanel) parent;
        fillTable(klsv.getAll());
        setLocationRelativeTo(null);
    }
    
    void fillTable(List<KhoiLuong> listKhoiLuong) {
        model = (DefaultTableModel) Table.getModel();
        model.setRowCount(0);
        int i=0;
        for (KhoiLuong khoiLuong : listKhoiLuong) {
            i++;
            model.addRow(new Object[]{i,khoiLuong.getMaKhoiLuong(),khoiLuong.getTenKhoiLuong()});
        }
        
    }
    
    void showData(int index) {
        KhoiLuong kl = klsv.getAll().get(index);
        txtTen.setText(kl.getTenKhoiLuong());
    }
    //Tự Động GenerateMa
    private String gennerateMa() {
        KhoiLuong sp = klsv.selectTop1DESC();
    ...
    private KhoiLuong readForm() {
        ...
        if (count == 0) {
            return new KhoiLuong(ma, ten, Auth.idNhanVien(), false);
        } else {
            return null;
        }
    }
    private KhoiLuong updateForm(){
        LocalDate toDay=this.getToDay();
        return new KhoiLuong(txtTen.getText(),toDay,Auth.idNhanVien());
    }
    void setForm(KhoiLuong kl){
        txtTen.setText(kl.getTenKhoiLuong());
    }
    void clearform(){
        setForm(new KhoiLuong());
    }
    ...
    private void initComponents() {
        ...
        jLabel4.setText("Thêm Khối Lượng");
        ...
        Table.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "STT", "Mã Khối Lượng", "Tên Khối Lượng"
            }
        ));
        ...
        jLabel2.setText("Tên Khối Lượng");
    ...
    private void btnThemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnThemActionPerformed
        // TODO add your handling code here:\
        KhoiLuong sp = readForm();
        if (sp != null) {
            int confirm = JOptionPane.showConfirmDialog(this, "Bạn Có Muốn Thêm Không?");
            if (confirm == 0) {
                if (klsv.addKhoiLuong(sp) > 0) {
                    fillTable(klsv.getAll());
                    spj.fillComboBoxKhoiLuong();
                    txtTen.getText();
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                KhoiLuongJDialog dialog = new KhoiLuongJDialog(new javax.swing.JPanel(), true);
