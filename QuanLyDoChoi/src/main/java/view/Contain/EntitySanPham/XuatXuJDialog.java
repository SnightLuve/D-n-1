...
public class XuatXuJDialog extends javax.swing.JDialog {
    ...
    XuatXuServiceImpl xxsv = new XuatXuServiceImpl();
    SanPhamJPanel spj;
    public XuatXuJDialog(JPanel parent, boolean modal) {
        initComponents();
        spj= (SanPhamJPanel)parent;
        fillTable(xxsv.getAll());
        setLocationRelativeTo(null);
    }

    void fillTable(List<XuatXu> listXX) {
        model = (DefaultTableModel) Table.getModel();
        model.setRowCount(0);
        int i=0;
        for (XuatXu xx : listXX) {
            i++;
            model.addRow(new Object[]{i,xx.getMaXuatXu(),xx.getTenXuatXu()});
        }

    }

    void showData(int index) {
        XuatXu xuatXu = xxsv.getAll().get(index);
        txtTen.setText(xuatXu.getTenXuatXu());
    }
    //Tự Động GenerateMa
    private String gennerateMa() {
        XuatXu sp = xxsv.selectTop1DESC();
    ...
    private XuatXu readForm() {
        ...
        if (count == 0) {
            return new XuatXu(ma, ten, Auth.idNhanVien(), false);
        } else {
            return null;
        }
    }
    private XuatXu updateForm(){
         LocalDate toDay=this.getToDay();
         return new XuatXu(txtTen.getText(),toDay,Auth.idNhanVien());
         
    }

    void setForm(XuatXu xuatxu) {
        txtTen.setText(xuatxu.getTenXuatXu());
    }
    ...
    private void btnThemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnThemActionPerformed
        ...
        if (sp != null) {
            ...
            if (confirm == 0) {
                if (xxsv.addXuatXu(sp) > 0) {
                    fillTable(xxsv.getAll());
                    spj.fillComboBoxXuatXu();
                    clearForm();
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                XuatXuJDialog dialog = new XuatXuJDialog(new javax.swing.JPanel(), true);
